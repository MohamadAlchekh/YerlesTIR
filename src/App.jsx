...
function App() {
  ...
  const handleAddItem = (e) => {
    ...
    if (w > maxDimension || h > maxDimension || d > maxDimension) {
      alert(
        "Hata: Girdiğiniz ürün ölçüleri konteynere sığmayacak kadar büyük!",
      );
      return;
    }
    ...
    if (count > 200 || items.length + count > 300) {
      alert(
        "Hata: Performans sorunu yaşamamak için toplam ürün sayısını 300 ile sınırlandırınız.",
      );
      return;
    }
  ...
  const handlePack = async () => {
    // puq.ai API'sine istek gönder
    const response = await fetch("https://api.puq.ai/h/bb9da2ee3138/sync", {
      method: "POST",
  ...
  const handleAnimationComplete = () => {
    if (
      isPlaying &&
      packResult &&
      animatingIndex < packResult.placed.length - 1
    ) {
      setAnimatingIndex((prev) => prev + 1);
    } else if (
      isPlaying &&
    ...
    <div className="app-wrapper">
      ...
      <div className="top-nav glass-panel">
        <div className="logo-container" style={{ margin: 0 }}>
          ...
          <h1
            style={{ marginBottom: 0, fontSize: "1.2rem", marginRight: "2rem" }}
          >
            Diginova BinPacker
          </h1>
        </div>
          ...
          <>
            <div className="sidebar glass-panel" style={{ width: "400px" }}>
              ...
              <form onSubmit={handleAddItem}>
                ...
                <div style={{ display: "flex", gap: "0.5rem" }}>
                  <input
                    ...
                    value={newItem.w}
                    onChange={(e) =>
                      setNewItem({ ...newItem, w: e.target.value })
                    }
                    className="flex-1"
                  />
                  <input
                    ...
                    value={newItem.h}
                    onChange={(e) =>
                      setNewItem({ ...newItem, h: e.target.value })
                    }
                    className="flex-1"
                  />
                  <input
                    ...
                    value={newItem.d}
                    onChange={(e) =>
                      setNewItem({ ...newItem, d: e.target.value })
                    }
                    className="flex-1"
                  />
                </div>
                ...
                <label className="checkbox-group" style={{ marginTop: "1rem" }}>
                  <input
                    ...
                    onChange={(e) =>
                      setNewItem({ ...newItem, stackable: e.target.checked })
                    }
                  />
              ...
              <button
                ...
                style={{ background: "var(--success)", marginTop: "1rem" }}
              >
            ...
            <div
              className="main-view glass-panel"
            ...
            >
              ...
              {items.length === 0 ? (
                <div className="empty-state">
                  Henüz ürün eklenmedi. Örnek CSV formatı: <br />
                  <br />
        ...
        {view === "optimization" && packResult && (
          <>
            <div className="sidebar glass-panel">
              ...
              <div
                className="instruction-panel glass-panel"
                style={{
                  marginTop: "1.5rem",
                  padding: "1rem",
              ...
              >
                ...
                {animatingIndex >= 0 &&
                animatingIndex < packResult.placed.length ? (
                  <div>
                    <div
                      style={{
                        fontWeight: "bold",
                    ...
                    >
                      {animatingIndex + 1}.{" "}
                      {packResult.placed[animatingIndex].name}
                    </div>
                    <div
                      style={{
                        fontSize: "0.9rem",
                    ...
                    >
                      Şimdi bu paketi alın ve şekilde gösterilen konuma
                      yerleştirin.
                      <br />
                      <br />
                      ...
                      {packResult.placed[animatingIndex].w} x{" "}
                      {packResult.placed[animatingIndex].h} x{" "}
                      {packResult.placed[animatingIndex].d} cm
                    </div>
                  </div>
                ) : (
                  <div
              ...
              <div style={{ marginBottom: "1.5rem" }}>
                ...
                >
                  ...
                  <span style={{ fontWeight: "bold", color: "var(--success)" }}>
                    {Math.round(
                      (checkedItems.size / packResult.placed.length) * 100,
                    )}
                    %
                  </span>
                </div>
                <div className="progress-bar" style={{ height: "8px" }}>
                  <div
                    className="progress-fill"
                    style={{
                      width: `${(checkedItems.size / packResult.placed.length) * 100}%`,
                    ...
                    <div className="check-info">
                      <div
                        style={{
                          fontWeight: "bold",
                          fontSize: "1rem",
                          color: checkedItems.has(idx)
                            ? "var(--text-muted)"
                            : "var(--text-main)",
                          textDecoration: checkedItems.has(idx)
                            ? "line-through"
                            : "none",
                        }}
                      >
                      ...
                      >
                        {item.w}x{item.h}x{item.d} cm | Konum: (
